...
Ecto-\/\+Trigger comprises a basic \href{https://rossgardiner.github.io/ecto-trigger/html/annotated.html}{\texttt{collection of files}}. Where each file defines a class and command-\/line interface for independent tasks. In other words, it is made up of a small number of Python scripts, that each do one job. Ecto-\/\+Trigger contains files for training a model, evaluating its performance, preparing to run it on a field device or computing an example saliency map for a given model and image.
...
\begin{DoxyItemize}
\item {\ttfamily train\+\_\+data\+\_\+dir} is the path to your directory of training images and YOLO-\/format labels
\item {\ttfamily val\+\_\+data\+\_\+dir} is the path to your directory of validation images and YOLO-\/format labels
\item {\ttfamily batch\+\_\+size} is the number of images per training step (e.\+g. 16)
\item {\ttfamily input\+\_\+shape} is the dimensions for the input size, specified as a string tuple, e.\+g. (120, 160, 3), height, width, number of channels. ~\newline

\item {\ttfamily alpha} controls the model size through changing the model width -\/ smaller values of alpha produces lighter models, but this has an accuracy penalty, see our paper for further details.
\item {\ttfamily log\+\_\+dir} is the directory where training logs will be saved for analysis with Tensor\+Board.
\item {\ttfamily epochs} is the number of full passes though the training data to run before exiting training.
...
\end{DoxyEnumerate}\hypertarget{md_guides_2usage_autotoc_md39}{}\doxysection{\texorpdfstring{Common Bugs and How to Address Them}{Common Bugs and How to Address Them}}\label{md_guides_2usage_autotoc_md39}

\begin{DoxyEnumerate}
\item {\bfseries{Dataset Format Issues}}:\+ Ensure your dataset is in YOLO annotation format. Incorrect formatting can cause errors during data loading, refer to the notes on YOLO annoation format above to help with this.
\item {\bfseries{Class Imbalance}}:\+ Imbalanced datasets can lead to poor model performance. Balance your dataset with equal parts of positive (images containing your object of interest) and negative (background image) samples.
...
\item {\bfseries{Quantisation Errors}}:\+ It is normal for quantisation to cause a slight degredation in model accuracy, but if this is large, consider ensuring your representative dataset is diverse.
